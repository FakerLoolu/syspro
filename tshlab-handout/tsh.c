...
void eval(char *cmdline) 
{
	pid_t pid;
	int bg;
	char *argv[MAXARGS];//command save
	bg = parseline(cmdline,argv);//command seperate   
	struct job_t *job;
	sigset_t mask;

	if(!builtin_cmd(argv)){ //If command has  builtin_cmd function
      sigemptyset(&mask);
      sigaddset(&mask,SIGCHLD);
		...
		if(execve(argv[0],argv,environ) < 0) { //correct run(?) check
	 ...
	 if(!bg) { 
		...
		else{kill(-pid,SIGINT);}; //kill child
	}
	else 
	{
...
void do_bgfg(char **argv, int output_fd) 
{
	...
	else
	{
		...
		if((jid=getjobpid(jobs,atoi(argv[1])))==NULL)
		{
